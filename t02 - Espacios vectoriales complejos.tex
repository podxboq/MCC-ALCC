...
Los espacios vectoriales se concibieron como una herramienta de geometría afín gracias a la introducción de coordenadas en el plano o espacio tridimensional.
...
\begin{itemize}
  \item Comprobar si cierto conjunto dotado de dos operaciones es o no espacio vectorial, así como verificar que cierto subconjunto es subespacio.
  \item Razonar si una colección de vectores es linealmente independiente y si conforma o no un sistema generador o una base de cierto espacio vectorial.
...
Esta base teórica será fundamental para comprender en los próximos temas la manipulación cde estados cuánticos como vectores.
...
Por regla general, siempre usamos la siguiente notación, con independencia de cuales son los componentes del cuerpo o de la operación que le da su estructura de cuerpo.
\begin{enumerate}
...
Pero no todos los cuerpos son numéricos, si consideramos un polinomio de grado 1 $p(x)=x-\alpha$ con $\alpha\in\R^*$, entonces bajo la relación de equivalencia de la aritmética residual, el conjunto conciente $\R[X]/ p(x)$ es un cuerpo.
...
\begin{eje}
  ...
  \begin{align*}
    (x_1,\dots, x_n)
    + ( y_1,\dots, y_n) & = ( x_1 + y_1,\dots, x_n + y_n) \\
    k(x_{1},\dots,x_n)  & = (kx_1,\dots, kx_n)
  \end{align*}
  para $k \in\C$ y $(x_1, \dots, x_n), (y_1,\dots, y_n)\in\C^n$.
\end{eje}
...
\begin{eje}
  El conjunto de matrices $M_{m \times n}(\K)$.
\end{eje}
...
Además no vamos a volver a repetir que el espacio vectorial es sobre \(\C\), por lo que cuando hablemos de espacio vectorial, nos estaremos refiriendo a un \(\C\)-espacio vectorial.
...
\begin{theo}[Caracterización de subespacios]
  ...
  $$\alpha u + \beta v \in W \quad \text{para todos } u, v \in W \text{ y } \alpha, \beta \in \C$$
\end{theo}
...
\begin{eje}
  Consideremos el espacio vectorial $\C^3$ y los vectores $v_1 = (1, 0, i)$ y $v_2 = (i, 1, 0)$, el subespacio vectorial generado por $S = \{v_1, v_2\}$ es
  $$\text{gen}(S) = \left\{\alpha (1, 0, i) + \beta (i, 1, 0) \mid \alpha, \beta \in \C\right\} = \left\{( \alpha + \beta i, \beta, \alpha i) \mid \alpha, \beta \in \C\right\}\,.$$
\end{eje}
...
\begin{eje}
  Determinar si los vectores $v_1 = (1, i, 0)$, $v_2 = (i, 0, 1)$, $v_3 = (1+i, i, 1)$ de $\C^3$ son linealmente independientes.

  \textbf{Solución:} Planteamos la ecuación
  $$\alpha_1 (1, i, 0) + \alpha_2 (i, 0, 1) + \alpha_3 (1+i, i, 1) = (0, 0, 0)\,,$$
...
\begin{prop}
  \begin{enumerate}
    ...
    \item Si un conjunto, tiene un subconjunto de vectores linealmente dependiente, entonces el conjunto es linealmente dependiente.
    \item Si un conjunto es linealmente independiente, entonces cualquier subconjunto también lo es.
...
\begin{eje}[Base canónica de $\C^n$]
  ...
  Cualquier vector $v = (v_1, v_2, \ldots, v_n) \in \C^n$ se puede escribir como:
  $$v = v_1 e_1 + v_2 e_2 + \cdots + v_n e_n\,.$$
\end{eje}
\begin{prop}
  Sea $V$ un espacio vectorial y $\mathcal{B}_1$ y $\mathcal{B}_2$ dos bases de $V$. Entonces, $\mathcal{B}_1$ y $\mathcal{B}_2$ tienen el mismo número de elementos.
\end{prop}
...
\begin{theo}[Existencia y unicidad de representación]
  ...
  Los escalares $\alpha_1, \alpha_2, \ldots, \alpha_n$ se llaman las coordenadas de $v$ respecto a la base $\mathcal{B}$.
\end{theo}
...
El concepto de dimensión es fundamental en álgebra lineal, ya que proporciona una medida del tamaño o grado de libertad del espacio vectorial. Un espacio vectorial puede ser de dimensión finita o infinita, dependiendo de si tiene una base con un número finito o infinito de vectores. En este curso solo consideraremos espacios vectoriales de dimensión finita y así daremos por entendido cada vez que se mencione una base o dimensión.
...
\begin{eje}
  ...
  $$W = \left\{(x, y, z, w) \in \C^4 \mid x + y - z = 0, \, 2x - y + w = 0\right\}$$
  ...
  \end{align*}
  Por tanto, los vectores en $W$ tienen la forma $(x, y, x+y, y-2x)$ que podemos expresar como combinación lineal de dos vectores
  $$(x, y, x+y, y-2x) = x(1, 0, 1, -2) + y(0, 1, 1, 1)$$
  Esta expresión muestra que cualquier vector en $W$ puede generarse a partir de los vectores $(1, 0, 1, -2)$ y $(0, 1, 1, 1)$. Es decir, $W = \text{gen}\{(1, 0, 1, -2), (0, 1, 1, 1)\}$.

  Por último tenemos que verificar que los vectores encontrados son linealmente independientes, para ello tomemos una combinación lineal que dé el vector cero
  \[
    \alpha(1, 0, 1, -2) + \beta(0, 1, 1, 1) = (0, 0, 0, 0)\,.
  \]
...
Una de las razones por las cuales es importante el concepto de base de un espacio vectorial, es porque de esta manera podemos caracterizar cualquier elemento del espacio vectorial en función de los coeficientes usados al ser expresado como combinación lineal de los elementos de la base.

A las coordenadas \(v_{1},\dots, v_{n}\) de \(v \in\V\) en la base $\mathcal{B}$  las representamos como un vector de $\C^n$ donde usaremos la notación \({v = (v_{1},\dots,v_{n})}_{\mathcal{B}} \in \C^n\).
Cuando no se exprese la base, estamos queriendo decir que usamos la base canónica de $\V$.
...
\begin{nota}
  Aunque se ha definido el concepto de base como un conjunto, en la práctica es habitual considerar las bases como listas ordenadas de vectores, pues el orden de los vectores en la base determina el orden de las coordenadas de un vector y este orden es importante en muchos contextos.
\end{nota}
...
Siguiendo con el ejemplo anterior, en la base usual \( B=\left\{ e_{1},\ e_{2} \right\}\) la malla consiste en los ejes coordenados usuales (Figura~\ref{fig:bases-rejilla}, izquierda), mientras que en la base \(B^{\prime} = \left\{ b_{1},\ b_{2} \right\}\) la dirección vertical de la malla tiene un ángulo menor de 90 grados (Figura~\ref{fig:bases-rejilla}, derecha).
...
\begin{eje}
  ...
  Las coordenadas de la matriz \(\begin{pmatrix}
    2    & - i & 4 \\
    2+3i & - 1 & 1
  \end{pmatrix}\) en la base usual es \((2, - i,4,2+3i, - 1,1)\).
\end{eje}
...
\begin{defi}
  ...
  \]
  Llamamos \textbf{matriz de cambio de base} o \textbf{matriz de cambio de coordenadas} de la base $\mathcal{B}$ a la base $\mathcal{B}^\prime$ a la matriz $M_{\mathcal{B}}^{\mathcal{B}^\prime}=(a_{ij})$.
\end{defi}

El motivo de llamar a $M_{\mathcal{B}}^{\mathcal{B}^\prime}$ la matriz cambio de base, se debe a que dado $v$ un vector de $\V$ con coordenadas $(x_1,\dots,x_n)_{\mathcal{B}}$ y coordenadas $(y_1,\dots,y_n)_{\mathcal{B}^\prime}$, tenemos
\begin{align*}
  v                                    & =\sum_{i=1}^n x_i v_i=\sum_{i=1}^n x_i \left( \sum_{j=1}^n a_{ij}v_j^\prime\right) =\sum_{j=1}^n \left( \sum_{i=1}^n x_i a_{ij}\right) v_j^\prime\Rightarrow \\
  y_j                                  & = \sum_{i=1}^n x_i a_{ij} = (x_1,\dots,x_n)_{\mathcal{B}}(M_{\mathcal{B}}^{\mathcal{B}^\prime})^j\Rightarrow                                                 \\
  (y_1,\dots,y_n)_{\mathcal{B}^\prime} & = (x_1,\dots,x_n)_{\mathcal{B}}M_{\mathcal{B}}^{\mathcal{B}^\prime}
\end{align*}
La ecuación matricial del cambio de base de $\mathcal{B}$ a $\mathcal{B}^\prime$ permite calcular las coordenadas de un vector en la base $\mathcal{B}^\prime$ conociendo las coordenadas con respecto a la base $\mathcal{B}$.

El cambio de base en el sentido contrario, es decir, calcular las coordenadas de un vector en la base $\mathcal{B}$ conociendo sus coordenadas en la base $\mathcal{B}^\prime$ viene dado por la matriz inversa.

\begin{eje}
  Consideremos en \(\mathbb{C}^{2}\) las siguientes bases, $\mathcal{B}=\set{(1,0), (i,2)}$ y $\mathcal{B}^{\prime}=\set{(2,3), (-1,2)}$.

  Para calcular la matriz de cambio de base de $\mathcal{B}^\prime$ a $\mathcal{B}$ debemos primero conocer cuál es la expresión de los vectores de $\mathcal{B}^\prime$ en función de los vectores de $\mathcal{B}$.
  ...
  \begin{align*}
    ...
    (-1,2) & = c(1,0)+d(i,2).
  \end{align*}
  La solución es \(a = 2-\frac{3}{2}i,\, b = \frac{3}{2}\) por lo que \((2,3) = {(2-\frac{3}{2}i,\frac{3}{2})}_{\mathcal{B}}\) y \(c = -1-i,\ d = 1\), por lo que \((-1,2) = {(-1-i,1)}_{\mathcal{B}}\).

  La matriz de cambio de base de $\mathcal{B}^\prime$ a $\mathcal{B}$ es
  \[
    M_{\mathcal{B}^\prime}^{\mathcal{B}} = \begin{pmatrix}
      2-\frac{3}{2}i & \frac{3}{2} \\
      -1-i           & 1
    \end{pmatrix}.
  \]
  Hagamos la comprobación, en la base $\mathcal{B}^\prime$ tenemos que \((-1,2) = 0 \cdot (2,3) + 1 \cdot ( - 1,2)\) por lo que
  \((-1,2) = (0,1)_{\mathcal{B}^\prime}\).
  Aplicando a este vector la matriz $M_{\mathcal{B}^\prime}^{\mathcal{B}} $ tenemos que
  \[
    (0,1) =
    \begin{pmatrix}
      2-\frac{3}{2}i & \frac{3}{2} \\
    ...
    \end{pmatrix} = (-1-i, 1).
  \]
  En otras palabras, \((-1,2) = (0,1)_{\mathcal{B}^\prime} = (-1-i, 1)_{\mathcal{B}}\).
\end{eje}
...
\begin{eje}
  Consideremos en \(\mathbb{C}^{3}\) los subespacios
  \[
  ...
  Por último, consideremos el vector \(v = (2,3,4)\). Podemos escribirlo como suma de un vector de \(U\) y otro de \(W\):
  \[
    v = (2,3,4) = (1,2,-3) + (1,1,1)
  \]
...
En los siguientes temas, daremos más detalles sobre como construir estos subespacios y que propiedades deben cumplir, así como obtener aquellos subespacios que nos permita obtener una representación directa del espacio vectorial con los espacios cuánticos.