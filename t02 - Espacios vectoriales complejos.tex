...
\begin{itemize}
  ...
  \item Identificar cuando un subconjuto de un espacio vectoria, hereda también la estructura de espacio vectorial.
...
Siempre usamos la siguiente notación, con independencia de cuales son los elementos del cuerpo o de la operación que le da su estructura de cuerpo.
\begin{enumerate}
...
Algunas veces se tiende a confundir el concepto de vector con el de coordenada, pues en la práctica, se escriben igual y se usan indistintamente, pero siendo formales, son conceptos distintos. Para diferenciar entre ambas situaciones, cuando trabajemos con vectores en $\C^n$ usaremos la notación del vector como una matriz columna, y cuando trabajemos con coordenadas usaremos la notación de las coordenadas como matriz fila.
...
\begin{defi}[Espacio vectorial]
  ...
  Los elementos del espacio vectorial $\V$ son \textbf{vectores} y los elementos del cuerpo $\K$ son \textbf{escalares}.
\end{defi}
...
\begin{defi}
  ...
  El subconjunto $U \subseteq\V$ es un \textbf{subespacio vectorial} de $\V$ si:
  \begin{enumerate}
...
\begin{eje}
  Consideremos en $\C^3$ el conjunto $S$ formado por los vectores $v_1 = \mqty(1\\ 0\\ i)$ y $v_2 = \mqty(i\\ 1\\ 0)$, el subespacio vectorial generado por $S$ es
...
\begin{eje}
  Para determinar si los vectores $v_1 = \mqty(1\\ i\\ 0)$, $v_2 = \mqty(i\\ 0\\ 1)$ y $v_3 = \mqty(1+i\\ i\\ 1)$ de $\C^3$ son linealmente independientes, planteamos la ecuación
...
\begin{eje}[Base canónica de $\C^n$]
  ...
  Cualquier vector $v = \mqty(v_1\\ v_2\\ \vdots\\ v_n) \in \C^n$ se puede escribir como
  $$v = v_1 e_1 + v_2 e_2 + \cdots + v_n e_n\,.$$

  La independencia lineal de $\{e_1, e_2, \ldots, e_n\}$ se debe a que la única solución de la ecuación
  $$\alpha_1 e_1 + \alpha_2 e_2 + \cdots + \alpha_n e_n = 0\,,$$
  es $\alpha_1 = \alpha_2 = \cdots = \alpha_n = 0$.

  Por lo tanto, $\{e_1, e_2, \ldots, e_n\}$ es una base de $\C^n$.
\end{eje}
...
\begin{eje}
  Sea $V$ el espacio vectorial de los polinomios de grado menor o igual que 2. La base canónica de $V$ es $\{1, x, x^2\}$. El vector $p(x) = 3 + 2x + x^2$ se puede escribir como
  \[
    p(x) = 3\cdot 1 + 2\cdot x + 1\cdot x^2\,.
  \]
  Por lo tanto, las coordenadas de $p(x)$ respecto de la base canónica son $(3, 2, 1)$.

  Si consideramos la base $\mathcal{B}=\{1, 1+x, 1+x+x^2\}$, el vector $p(x)$ se puede escribir como
  \[
    p(x) = 1\cdot 1 + 1\cdot (1+x) + 1\cdot (1+x+x^2)\,.
  \]
  Por lo tanto, las coordenadas de $p(x)$ respecto de la base $\mathcal{B}$ son $(1, 1, 1)_{\mathcal{B}}$.
  \[
    p(x) = (3, 2, 1) = (1, 1, 1)_{\mathcal{B}}\,.
  \]
\end{eje}

Cuando en las coordenadas de un vector no se especifica la base, se está utilizando la base canónica, haciendo que las coordenadas coincidan con la representación del vector como vector fila, dando lugar algunas veces a confusión.

\begin{prop}
  Sea $V$ un espacio vectorial de dimensión finita y $\mathcal{B}_1$ y $\mathcal{B}_2$ dos bases de $V$. Entonces, $\mathcal{B}_1$ y $\mathcal{B}_2$ tienen el mismo cardinal.
\end{prop}
\begin{proof}
...
\begin{defi}[Dimensión]
  La \textbf{dimensión} de un espacio vectorial $V$ es el número de vectores en cualquiera de sus bases y se denota por $\dim(V)$.
\end{defi}
...
\begin{defi}
  ...
  \]
  La \textbf{matriz cambio de coordenadas} de $\mathcal{B}$ a $\mathcal{B}^\prime$ es la matriz $M_{\mathcal{B}}^{\mathcal{B}^\prime}=(a_{ij})$.
\end{defi}
...
\begin{eje}
  Sean $U = \left\langle (1,0) \right\rangle$ y $W = \left\langle (0,1) \right\rangle$ subespacios vectoriales de $\C^2$. La unión de $U$ y $W$ no es un subespacio vectorial, ya que
  \[
    (1,0) + (0,1) = (1,1) \notin U \cup W\,.
  \]
\end{eje}
...
\begin{defi}[Suma de subespacios]
  Sea $\V$ un espacio vectorial y $U,W$ dos subespacios vectoriales de $\V$. La \textbf{suma} de $U$ y $W$ es
  \[
...
\begin{defi}
  ...
  La suma de $U$ y $W$ es \textbf{directa} si \(U \cap W = \{0\}\). Se denota como \(U \oplus W\).
\end{defi}
...
\end{prop}
\begin{proof}
  Este resultado es consecuencia del resultado anterior.
\end{proof}
