...
\begin{theo}[Existencia y unicidad de representación]
  ...
  Los escalares $\alpha_1, \alpha_2, \ldots, \alpha_n$ se llaman las \textbf{coordenadas} de $v$ respecto a la base $\mathcal{B}$.
\end{theo}
...
\begin{eje}
  ...
  Las coordenadas de la matriz \(\begin{pmatrix}
    2    & - i & 4 \\
    2+3i & - 1 & 1
  \end{pmatrix}\) en la base usual es \((2, - i,4,2+3i, - 1,1)\) y que como podemos observar es un espacio vectorial de dimensión $6$.
\end{eje}
...
\begin{prop}
  Sea $\V$ un espacio vectorial de dimensión $n$ y sean $\mathcal{B}$ y $\mathcal{B}^\prime$ dos bases de $\V$. La matriz cambio de base de $\mathcal{B}^\prime$ a $\mathcal{B}$ es la inversa de la matriz de cambio de base de $\mathcal{B}$ a $\mathcal{B}^\prime$
  \[
    M_{\mathcal{B}}^{\mathcal{B}^\prime} = \left(M_{\mathcal{B}^\prime}^{\mathcal{B}}\right)^{-1}\,.
  \]
\end{prop}
