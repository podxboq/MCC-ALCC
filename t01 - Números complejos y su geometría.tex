...
\begin{esquemaExplorador}
  \temaEsquema{Fundamentos}{
    ...
    \temaEsquema{Formas}{
      \conceptoEsquema{Cartesiana}{$(a,b)$}
      \conceptoEsquema{Binomial}{$a + bi$}
      \conceptoEsquema{Polar}{$r(\cos(\theta) + i\sin(\theta))$}
      \conceptoEsquema{Exponencial}{$re^{i\theta}$}
    }
  }
  \temaEsquema{Relaciones}{
    \conceptoEsquema{Desigualdad triangular}{$|z+w| \leq |z| + |w|$}
  ...
  \temaEsquema{Funciones}{
    \conceptoEsquema{Exponencial}{$\sum_{n=0}^{\infty} \frac{z^n}{n!}$}
    \conceptoEsquema{Seno}{$\frac{e^{iz} - e^{-iz}}{2i}$}
    \conceptoEsquema{Coseno}{$\frac{e^{iz} + e^{-iz}}{2}$}
    \conceptoEsquema{Seno hiperbólico}{$\frac{e^z - e^{-z}}{2}$}
    \conceptoEsquema{Coseno hiperbólico}{$\frac{e^z + e^{-z}}{2}$}
  }
...
\begin{prop}
  ...
  \begin{enumerate}
    ...
    \item \textbf{Distributiva:} dados $z,w,x\in\C$, $z(w+x) = zw + zx$.
...
\begin{eje}
  Los elementos especiales identificados en el resultado~\ref{prop:propiedades_cuerpo} que demuestran que $\C$ es un cuerpo conmutativo son:
  \begin{itemize}
    ...
    \item El elemento opuesto para el producto es $z^{-1} = (\dfrac{a}{a^2+b^2}, \dfrac{-b}{a^2+b^2})$.
  \end{itemize}
\end{eje}
...
Vamos a describir los elementos de $\C$ de forma más cómoda, dándole entidad propia y sin dependencia de la identificación con un elemento del plano.
...
\begin{itemize}
  ...
  \item \textbf{Circunferencias}:
        \[ C(z_0,r) = \{ z\in\C\ \vert\ d(z_0,z) = r \} = \partial D(z_0,r)\,. \]
        En particular, la circunferencia unidad $C_1 = \partial D(0, 1)= \{z\in\C\ \vert\ \abs{z}=1\}$.
...
La función exponencial compleja, cumple las mismas propiedades que la exponencial real en cuanto a las manipulaciones algebraicas, como vemos en el siguiente resultado, cuya demostración se basa en la fórmula de Euler~\eqref{eq:formula-euler}.
