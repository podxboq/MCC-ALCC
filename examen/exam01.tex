\begin{questions}
  ...
  \question[3] Considere un sistema de dos cúbits. Dado el estado
  ...
  \begin{parts}
    ...
    \part Calcule la matriz de densidad $\rho = |\psi\rangle\langle\psi|$ y determine la matriz de densidad reducida $\rho_A$ para el primer cúbit.
    \part ¿Qué información nos proporciona $\rho_A$ sobre el estado del primer cúbit?
  \end{parts}
  ...
  \begin{solution}
    ...
    d) La matriz de densidad reducida $\rho_A$ nos muestra que el primer cúbit está en un estado mixto con igual probabilidad (50\%) de estar en $|0\rangle$ o $|1\rangle$. También podemos ver que $\rho_A = |\phi_A\rangle\langle\phi_A|$ donde $|\phi_A\rangle = \frac{1}{\sqrt{2}}(|0\rangle + |1\rangle)$, lo que confirma que el estado global es separable.

    Además, $Tr(\rho_A^2) = 1$, lo que indica que el primer cúbit está en un estado puro, como es de esperar para un estado global separable.
  \end{solution}
\end{questions}