\begin{questions}
  ...
  \begin{parts}
    \part Defina la esfera de Bloch y explica como se puede representar un cúbit puro como un punto de dicha esfera.
    ...
    \begin{solution}
      ...
      1) Operadores sobre múltiples cúbits:
      - Si $X, Y, Z$ son matrices de Pauli, $X \otimes Z$ actúa como $X$ en el primer cúbit y $Z$ en el segundo
      - Compuertas de dos cúbits como CNOT se pueden expresar usando productos tensoriales
      - Para un sistema de $n$ cúbits, los operadores tienen forma $A_1 \otimes A_2 \otimes \cdots \otimes A_n$
    ...
    \begin{solution}
      ...
      - Ayuda a analizar el estado de cúbits específicos durante la ejecución de algoritmos
      ...
      - En computación cuántica de estado sólido, permite estudiar la interacción entre el cúbit y su entorno
  ...
  \begin{parts}
    ...
    \part  Obtenga la matriz de densidad reducida $\rho_A$ del primer cúbit en este caso particular.
  \end{parts}

  \begin{solution}
    a) Un estado de dos cúbits es separable si y solo si puede escribirse como el producto tensorial de dos estados de un cúbit:
    ...
    c) La matriz de densidad reducida $\rho_A$ del primer cúbit se obtiene tomando la traza parcial sobre el segundo cúbit:
    ...
    - $S(\rho_A) = 1$ es el valor máximo posible para un sistema de un cúbit, indicando un estado completamente mixto
    ...
    - Este resultado implica que no podemos predecir el resultado de una medición en el primer cúbit independientemente del segundo cúbit
  ...
  \begin{solution}
    ...
    - Esta compuerta es esencial en circuitos cuánticos para realizar rotaciones controladas de un solo cúbit
