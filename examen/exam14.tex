\begin{questions}
  ...
  \begin{parts}
    \part Explique el concepto de compuertas cuánticas, cómo se representan matemáticamente y describa las compuertas de un cúbit más importantes.
    ...
    \begin{solution}
      ...
      - Formalmente, una compuerta cuántica que actúa sobre $n$ cúbits se representa mediante una matriz unitaria $U$ de dimensión $2^n \times 2^n$
      ...
      Compuertas cuánticas de un cúbit:
      ...
      - Las compuertas de un cúbit más CNOT forman un conjunto universal

      2) Visualización en la esfera de Bloch:
      - Las compuertas de un cúbit corresponden a rotaciones en la esfera de Bloch
      ...
      - Las compuertas de un cúbit generalmente son más fáciles de implementar y tienen mayor fidelidad que las de múltiples cúbits
      ...
      Las compuertas cuánticas de un cúbit, junto con al menos una compuerta de dos cúbits (como CNOT), proporcionan todos los elementos necesarios para implementar cualquier algoritmo cuántico, constituyendo el alfabeto fundamental de la programación cuántica.
    \end{solution}
  ...
  \question[3] Considere el espacio vectorial complejo $\mathbb{C}^4$ con la base computacional, y sea el estado
  ...
  \begin{solution}
    a) Para determinar si el estado $|\psi\rangle = \alpha|00\rangle + \beta|11\rangle$ es separable o entrelazado, debemos verificar si puede expresarse como un producto tensorial de dos estados de un cúbit:
    ...
    c) Para obtener la matriz de densidad reducida $\rho_1$ del primer cúbit, tomamos la traza parcial sobre el segundo cúbit:
    ...
    Significado físico:
    La entropía de von Neumann de la matriz de densidad reducida $\rho_1$ cuantifica el grado de entrelazamiento entre el primer cúbit y el segundo. Sus propiedades e interpretación son:
    ...
    3) La entropía mide la "mezcla" o "impureza" del estado reducido, que surge debido al entrelazamiento con el otro cúbit.

    4) Físicamente, indica el grado de información compartida entre los dos cúbits. Un valor alto significa que no se puede obtener información completa sobre un cúbit sin considerar el otro.
  ...
  $|\Psi^+\rangle = \frac{1}{\sqrt{2}}(|01\rangle + |10\rangle)$ y $|\Psi^-\rangle = \frac{1}{\sqrt{2}}(|01\rangle - |10\rangle),$
  \begin{parts}
  ...
  \begin{solution}
    ...
    Esto demuestra que al aplicar CNOT a $|\Phi^+\rangle$ obtenemos un estado separable (no entrelazado), donde el primer cúbit está en el estado $\frac{1}{\sqrt{2}}(|0\rangle + |1\rangle) = |+\rangle$ y el segundo cúbit está en el estado $|0\rangle$.

    d) Los estados de Bell son fundamentales en el protocolo de teleportación cuántica, que permite transferir el estado desconocido de un cúbit a otro cúbit distante sin transportar físicamente el cúbit original. El proceso funciona de la siguiente manera:

    1) Recursos iniciales:
    - Alice posee un cúbit en un estado desconocido $|\psi\rangle = \alpha|0\rangle + \beta|1\rangle$ que desea teleportar a Bob
    - Alice y Bob comparten previamente un par de cúbits en un estado de Bell, típicamente $|\Phi^+\rangle = \frac{1}{\sqrt{2}}(|00\rangle + |11\rangle)$
    - Alice tiene acceso al primer cúbit del par entrelazado, Bob al segundo

    2) Medición de Bell:
    - Alice realiza una medición de Bell en los dos cúbits que posee: su cúbit original y su parte del par entrelazado
    - Esta medición proyecta los dos cúbits en uno de los cuatro estados de Bell: $|\Phi^+\rangle$, $|\Phi^-\rangle$, $|\Psi^+\rangle$ o $|\Psi^-\rangle$
    ...
    - Dependiendo del resultado recibido, Bob aplica una de cuatro operaciones diferentes a su cúbit:
    ...
    * Si Alice midió $|\Psi^-\rangle$: Bob aplica $ZX$
    ...
    5) Resultado:
    - Tras aplicar la operación correcta, el cúbit de Bob se encuentra exactamente en el estado original $|\psi\rangle = \alpha|0\rangle + \beta|1\rangle$
