\begin{questions}
  ...
  \begin{parts}
    \part Explique el concepto de producto tensorial entre espacios vectoriales complejos y cómo se aplica a los sistemas cuánticos de múltiples cúbits.
    ...
    \begin{solution}
      ...
      - El espacio de estados de un sistema de $n$ cúbits es $(C^2)^{\otimes n} \cong C^{2^n}$
      - Si $\{|0\rangle, |1\rangle\}$ es la base computacional para un cúbit, entonces
      ...
      es la base computacional para dos cúbits.
    ...
    \part Defina el concepto de núcleo de un operador lineal entre espacios vectoriales complejos. Demuestra que la dimensión de la imagen más la dimensión del núcleo es igual a la dimensión del espacio de partida (Teorema de la dimensión).

  \end{parts}

  \question[3] Considere las matrices de Pauli $X$, $Y$ y $Z$.
  \begin{parts}
    \part Compruebe que $XYZ = iI$.
    \part Las matrices de Pauli, junto con la identidad $I$, forman una base para el espacio de matrices complejas $2 \times 2$. Exprese la matriz $A = \begin{pmatrix} 1 & 1+i \\ 1-i & 1 \end{pmatrix}$ como combinación lineal de esta base.
  \end{parts}
  ...
  \question[3] Considere un sistema de dos cúbits en el estado
  \[|\psi\rangle = \frac{1}{\sqrt{2}}(|00\rangle + |11\rangle)\]
  \begin{parts}
    \part Demuestre que este estado está entrelazado.
    \part Calcule la matriz de densidad para este estado.
    \part Obtenga la matriz de densidad reducida para el primer cúbit y explique lo que indica sobre el estado de este cúbit.
  \end{parts}

  \begin{solution}
    a) Para demostrar que el estado está entrelazado, debemos verificar que no puede ser escrito como un producto tensorial de dos estados de un cúbit: $|\psi\rangle \neq |\phi_1\rangle \otimes |\phi_2\rangle$.
    ...
    c) La matriz de densidad reducida para el primer cúbit se obtiene tomando la traza parcial sobre el segundo cúbit:
    ...
    Esto indica que el primer cúbit está en un estado completamente mixto, con probabilidad 50\% de ser $|0\rangle$ y 50\% de ser $|1\rangle$. Este es un estado máximamente mixto (entropía máxima), característico de un cúbit que está completamente entrelazado con otro sistema.

    d) Si medimos el primer cúbit y obtenemos $|0\rangle$, el estado colapsará según el postulado de medición. Aplicando el proyector $P_0 = |0\rangle\langle0| \otimes I$ al estado:
    ...
    Por tanto, después de medir el primer cúbit y obtener $|0\rangle$, el sistema completo colapsará al estado $|00\rangle$, lo que significa que el segundo cúbit también estará en el estado $|0\rangle$. Esto demuestra la correlación perfecta entre los cúbits que es característica de este estado entrelazado.
  \end{solution}
\end{questions}