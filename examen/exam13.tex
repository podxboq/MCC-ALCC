\begin{questions}
  ...
  \begin{parts}
    \part Defina qué es la esfera de Bloch y explique cómo se utiliza para representar estados de un cúbit. Incluya la relación entre las coordenadas en la esfera y los coeficientes del estado cuántico.

    \begin{solution}
      La esfera de Bloch es una representación geométrica del espacio de estados puros de un cúbit, que permite visualizar y manipular estados cuánticos de dos niveles de manera intuitiva mediante un modelo tridimensional.

      Definición formal:
      La esfera de Bloch es una esfera unitaria en $\mathbb{R}^3$ donde:
      - Cada punto de la superficie representa un estado cuántico puro de un cúbit
      ...
      Relación con estados de cúbit:

      1) Parametrización general:
      Todo estado puro de un cúbit puede escribirse como:
      ...
      - Las compuertas cuánticas de un cúbit corresponden a rotaciones en la esfera de Bloch
      ...
      * $Z$: rotación de 180° alrededor del eje $z$
      ...
      5) Limitaciones:
      - Solo representa sistemas de un cúbit (estados de dos niveles)
      - No visualiza directamente el entrelazamiento entre múltiples cúbits
      - Para sistemas multi-cúbit se requieren extensiones o representaciones alternativas
    ...
    \part Describa los postulados de la mecánica cuántica en el contexto de la computación cuántica, en términos de espacios de Hilbert.
    ...
    \begin{solution}
      ...
      - Un cúbit se representa como un vector en $\mathbb{C}^2$: $|\psi\rangle = \alpha|0\rangle + \beta|1\rangle$
      - Un sistema de $n$ cúbits corresponde a un vector en $\mathbb{C}^{2^n}$
      - La superposición permite que un registro de $n$ cúbits represente simultáneamente $2^n$ estados computacionales clásicos
      - Esta representación exponencial es el origen del poder computacional cuántico

      Aplicaciones prácticas:
      - Inicialización: Los algoritmos comienzan típicamente con cúbits en estado $|0\rangle^{\otimes n}$
      ...
      Aplicaciones prácticas:
      - Lectura de resultados: Al final de algoritmos cuánticos se miden los cúbits para obtener el resultado clásico
      ...
      Interpretación en computación cuántica:
      - Un registro de $n$ cúbits ocupa un espacio de Hilbert de dimensión $2^n$
      - El entrelazamiento permite correlaciones no clásicas entre cúbits
      - Las operaciones en cúbits individuales afectan al estado global del sistema
      - El entrelazamiento es un recurso fundamental para ventajas cuánticas

      Aplicaciones prácticas:
      - Compuertas de múltiples cúbits: CNOT, Toffoli, etc., generan entrelazamiento
      - Algoritmo de Shor: El entrelazamiento entre registros de periodo y función permite la factorización eficiente
      - Codificación superdensa: Permite enviar 2 bits clásicos con 1 cúbit aprovechando un par entrelazado
      ...
      - Inicializar: Preparar cúbits en estados específicos (usualmente $|0\rangle^{\otimes n}$)
      ...
      * Entrelazamiento (correlaciones no locales entre partes del registro)
      ...
      - Sistema compuesto (Postulado 4): La difusión de Grover opera sobre todos los cúbits simultáneamente
  ...
  \question[3]
  Considere la compuerta Hadamard (H) y la compuerta de fase (S).
  \begin{parts}
  ...
  $|\psi_1\rangle = \frac{1}{\sqrt{2}}(|00\rangle + |11\rangle)$ y $|\psi_2\rangle = |0\rangle \otimes \frac{1}{\sqrt{2}}(|0\rangle + |1\rangle)$.
  \begin{parts}

    \part ¿Cuál de estos estados está entrelazado? Justifique su respuesta.
    \part Con respecto a la base computacional del espacio de Hilbert asociado, calcula la matriz que representa el estado $|\psi_1\psi_2\rangle$.
  \end{parts}
  ...
  \begin{solution}
    ...
    - Este es el valor máximo posible para dos cúbits, indicando entrelazamiento máximo
