\documentclass[]{unirTema}

\input{personalizacion}

\printanswers

\begin{document}
\author{Francisco Costa Cano}
\color{negrounir}
\titulacion{Máster en computación cuántica}
\asignatura{Álgebra lineal en computación cuántica}
%\bloque{1}{Fundamentos matemáticos}
%\tema{1}{Números complejos y su geometría}
%\input{t01 - Números complejos y su geometría}
%\input{t01 - a fondo}
%\input{t01 - problemas}
%\tema{2}{Espacios vectoriales complejos}
%\input{t02 - Espacios vectoriales complejos}
%\input{t02 - a fondo}
%\input{t02 - problemas}
%\tema{3}{Operadores lineales y representación matricial}
%\input{t03 - Operadores lineales y representación matricial}
%\input{t03 - a fondo}
%\input{t03 - problemas}
%\tema{4}{Espacios de Hilbert I}
%\input{t04 - Espacios de Hilbert I}
%\input{t04 - a fondo}
%\input{t04 - problemas}
%\tema{5}{Espacio dual y tensorial}
%\input{t05 - Espacio dual y tensorial}
%\input{t05 - a fondo}
%\input{t05 - problemas}
\bloque{2}{Espacios cuánticos}
%\tema{6}{Notación de Dirac}
%\input{t06 - Notación de Dirac}
%\input{t06 - a fondo}
%\input{t06 - problemas}
%\tema{7}{Operadores lineales en computación cuántica}
%\input{t07 - Operadores lineales en computación cuántica}
%\input{t07 - a fondo}
%\input{t07 - problemas}
\tema{8}{Estados probabilísticos}
\input{t08 - Estados probabilísticos}
\input{t08 - a fondo}
\input{t08 - problemas}
\end{document}