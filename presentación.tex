\documentclass{beamer}
\usetheme{Unir}
\usepackage{upgreek}
\usepackage{float}

\title{Álgebra lineal en computación cuántica}
\subtitle{Máster en computación cuántica}

\author[F. Costa Cano]{Dr. Francisco Costa Cano}
\institute[UNIR]{Universidad Internacional de La Rioja}
\date{Marzo 2026}

\begin{document}

\portada

\section{Calendario}
\begin{frame}
  \frametitle{\insertsection}
  \begin{itemize}
    \item Periodo formativo: 16/03/2026 - 26/06/2026.\pause
    \item Periodo repaso: 29/06/2026 - 03/07/2026.\pause
    \item Exámenes: 10/07/2026 - 15/07/2026.\pause
    \item Periodo vacacional: 30/03/2026 - 05/04/2026.
  \end{itemize}
\end{frame}


\section{Contenido formativo}
\begin{frame}
  \frametitle{\insertsection}
  \begin{itemize}
    \item 8 Temas.\pause
    \item 22 sesiones formativas.\pause
          \begin{itemize}
            \item 1 Sesión de presentación.\pause
            \item 15 sesiones de temario.\pause
            \item 4 Sesiones de repaso.\pause
            \item 1 Sesión de laboratorio.\pause
            \item 1 Sesión de examen.\pause
          \end{itemize}
    \item 3 actividades (2 individual, 1 en grupo).\pause
          \begin{itemize}
            \item Operaciones (27/04/2026).
            \item Producto tensorial y esfera de Bloch (18/05/2026).
            \item Matriz de densidad (08/06/2026).
          \end{itemize}\pause
    \item 8 test.
  \end{itemize}
\end{frame}


\section{Temario}
\begin{frame}
  \frametitle{Temario}
  \textbf{Bloque I.} Espacios vectoriales.
  \begin{enumerate}
    \item Números complejos y su geometría.
    \item Espacios vectoriales complejos.
    \item Operadores lineales y representación matricial.
  \end{enumerate}\pause
  \textbf{Bloque II.} Espacios de Hilbert.
  \begin{enumerate}
    \item Espacios de Hilbert.
    \item Espacio dual y tensorial.
  \end{enumerate}\pause
  \textbf{Bloque III.} Espacios cuánticos.
  \begin{enumerate}
    \item Notación de Dirac.
    \item Operadores lineales en computación cuántica
    \item Estados probabilísticos
  \end{enumerate}
\end{frame}

\section{Recursos}
\begin{frame}
  \begin{itemize}
    \item El temario nuevo en Documentación.
    \item Biblioteca UNIR.
    \item Foro.
    \item Internet y Youtube.\pause
    \item \textcolor{red}{OJO:} Las IA generativas no generan conococimiento, son herramientas de ayuda.
  \end{itemize}
\end{frame}

\section{Evaluación}

\begin{frame}
  \frametitle{Evaluación}
  \begin{itemize}
    \item Evaluación continua (40\%): Se pueden conseguir un máximo de 10 puntos de hasta 15 puntos disponibles. Solo suman las actividades con calificación mayor o igual a 5.\pause
          \begin{itemize}
            \item Asistencia a clase, 0.5 punto: 0.25 por clase.
            \item Tests, 1.2 puntos: 0.15 punto cada test.
            \item Actividades, 13 puntos: Actividad 1: 5 puntos, actividad 2: 5 puntos y actividad 3 (grupal): 3.3 puntos.\pause
          \end{itemize}
    \item Examen final (60\%): es necesario su aprobado (puntuación mayor o igual a 5)
  \end{itemize}
\end{frame}

\begin{frame}
  \frametitle{Ejemplos de evaluación}

  10 puntos en evaluación continua son 4 puntos para la nota.\pause
  \[
    \begin{cases}
      \text{5 puntos en examen son 3 puntos para la nota.}\pause  & \text{\textcolor{azul}{Aprobado (7)}}\pause     \\
      \text{4 puntos en examen son 2.4 puntos para la nota}\pause & \text{\textcolor{marrón}{Suspenso (2.4)}}\pause
    \end{cases}
  \]

  0 puntos en evaluación continua son 0 puntos para la nota.\pause
  \[
    \begin{cases}
      \text{8 puntos en examen son 4.8 puntos para la nota.}\pause & \text{\textcolor{marrón}{Suspenso (4.8)}}\pause \\
      \text{10 puntos en examen son 6 puntos para la nota}\pause   & \text{\textcolor{azul}{Aprobado (6)}}
    \end{cases}
  \]
\end{frame}

\begin{frame}
  \begin{center}
    \Huge\scshape\textcolor{texto}{Muchas\\gracias}
  \end{center}
\end{frame}
\end{document}
