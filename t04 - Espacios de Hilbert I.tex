...
\begin{prop}
  ...
  \begin{enumerate}
    \item $\langle u, \alpha v + \beta w \rangle = \conj{\alpha}\langle u, v\rangle + \conj{\beta}\langle u, w\rangle$.
    \item $\langle u, v \rangle = \conj{\langle v, u \rangle}$.
...
Nuestro objetivo es dotar a los espacios vectoriales de una estructura geométrica que nos permita definir conceptos como ángulos, longitudes y ortogonalidad. Para ello, necesitamos de un poderoso concepto, la distancia, que nos permitirá definir conceptos de proximidad e incluso dotar al espacio de una topología adecuada.
...
Por último, la norma nos permite definir el concepto de distancia, que es esencial para para los resultados de en el estudio de la información cuántica.
...
\begin{eje}
  Usando los ejemplos de producto interno del inicio del tema podemos tener los siguientes ejemplos de distancias:
  \begin{itemize}
    \item En $\C^n$: $d(u, v) = \sum_{k=1}^n  |u_k - v_k|^2$.

    \item En $\C^{m \times n}$: $d(A, B) = \sum_{i,j} |A_{ij} - B_{ij}|^2$.

    \item En $L^2([a,b])$: $d(f, g) = \int_a^b |f(x) - g(x)|^2 \, dx$.
  \end{itemize}
\end{eje}
