\begin{questions}
  ...
  \question La transformación $T: \C^2 \to \C^2$ definida por $T(x,y) = (x + iy, 2x)$ es:
  ...
  \end{choices}
  \begin{solution}
    Es lineal porque $T(\alpha u + \beta v) = \alpha T( u) + \beta T( v)$ para todos $ u,  v \in \C^2$ y $\alpha, \beta \in \C$.
  \end{solution}

  \question El núcleo de la transformación lineal $T(x,y,z) = (x + y, 2x + 2y)$ de $\C^3$ en $\C^2$ es:

  \begin{choices}
    \choice $\{0\}$
    \choice $\text{gen}\left\{(1,0,0)\right\}$
    \CorrectChoice $\text{gen}\left\{(-1,1,0), (0,0,1)\right\}$
    \choice Todo $\C^3$
  \end{choices}
  \begin{solution}
    Resolviendo $T( v) = 0$: $x + y = 0$ y $2x + 2y = 0$ implica $y = -x$ y $z$ libre. Base del núcleo: $\left\{(-1,1,0), (0,0,1)\right\}$.
  \end{solution}
  ...
  \end{choices}
  \begin{solution}
    Esta relación muestra que $H$ conjuga la matriz $X$ en $Z$, es decir, transforma una en la otra mediante cambio de coordenadas.
  \end{solution}
  ...
  \begin{choices}
    \choice $\dim(\Ker(T)) > 0$
    \CorrectChoice $\Ker(T) = \{0\}$
    \choice $\Ima(T) \neq V$
    \choice $[T]$ es hermitiana
  \end{choices}
  \begin{solution}
    Una transformación es invertible si y solo si es inyectiva, lo que ocurre cuando $\Ker(T) = \{0\}$.
  \end{solution}
